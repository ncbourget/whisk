...
Cindy’s local workflow adds `editor → loopback server → validation + backup → JSON + HTML`. The Cloudflare server module authenticates the hosted editor. With publishing explicitly enabled and a server-only repository credential, it reads and publishes content through the fixed GitHub repository. Otherwise it remains read-only. The public website remains static.
...
| `GET/HEAD /api/content` | Authorized current GitHub content + revision when publishing is enabled; otherwise bundled read-only snapshot | Authorized full snapshot + revision; `capabilities.write=true` |
| `POST /api/content` | Access JWT + exact email/host + same Origin, size/schema/URL/revision validation, atomic Git commit; otherwise 405 | Session AND exact same-origin/Host check, JSON type/size, revision, schema and URL validation, render preflight, backup, then save |
| `POST /api/photo` | Access JWT + exact email/host + same Origin, 5 MB cap, raster signature/type check, hash-named photo commit; otherwise 405 | Session AND same-origin/Host check, size/type/signature checks, safe filename, then local image write |
...
The two POST routes above, implemented independently for local and hosted mode, are the complete website write API. A content save can change menu items, events, business status, Square links, and site configuration; therefore ALL fields share the same server-side authorization. Image upload has its own equally enforced check. Import/download and add/remove/reorder controls only modify browser drafts until an authorized save or publish; editing client JavaScript cannot authorize a save. There are no other Workers, Pages Functions, webhook receivers, OAuth callbacks, catalog-sync endpoints, storage bindings, or committed GitHub write tokens in this repository.
...
`server/handler.mjs` authorizes every administrative request, then dispatches. Only POST /api/content and POST /api/photo can mutate content, and only when EDITOR_PUBLISH_ENABLED=true and GITHUB_CONTENT_TOKEN is configured. Other write methods/routes return 405. These POSTs require an exact Origin match; a supplied Sec-Fetch-Site must be same-origin. The UI capability flag grants no authority. Administrative responses use `private, no-store`, frame denial, no-referrer, a restrictive CSP, and no CORS grant. There are no service-token or development bypasses. Only one exact HTTPS administrative hostname is accepted; direct `pages.dev`, preview, branch aliases, or other custom-domain routes are denied unless that exact hostname is deliberately selected as the canonical host.
...
### Hosted publishing implementation

`server/content.mjs` validates exact content schemas, text/list limits, prices, time windows, Square-owned HTTPS links, and image references to regular repository files. Content requests are capped at 256 KiB; photo uploads at 5 MiB. Only JPEG/PNG/WebP signatures matching the declared MIME type are accepted; uploads cannot provide filenames, SVG, scripts or filesystem paths. Signature checks are not image transcoding or malware scanning.

`server/publish.mjs` always targets `ncbourget/whisk`, branch `main`, through `api.github.com`. Requests cannot choose a repository, branch or API URL. A content publish writes only `data/site.json`, `data/menu.json`, `data/events.json` in one Git tree/commit. Photos write only `assets/food/upload-<sha256>.<jpg|png|webp>`. The token is a Cloudflare runtime secret with repository Contents read/write only. GitHub commit history supplies the content audit trail; Cloudflare Access logs supply login attribution. No JWTs or credentials are logged.

The editor reads the current GitHub revision rather than a stale deployed snapshot. Saves compare the content revision, then advance main without force using the read commit as the parent. Concurrent pushes reject rather than overwrite. Unreferenced Git objects from failed updates are not published. An uncertain network result requires reload before retrying. Uploaded photos are committed immediately and may be visible in a public repository before their item is published. They become public site assets when referenced by visible content. Draft browser edits are not autosaved.

A successful commit triggers the existing Cloudflare Git build. It is not proof that deployment succeeded. The editor says the build is pending and asks the owner to verify the live site; failed builds leave the previous deployment serving. GitHub/Cloudflare service outages, branch restrictions, expired credentials and free quota limits must surface errors, never bypass authentication. To disable hosted writes, set EDITOR_PUBLISH_ENABLED=false and redeploy. The Git credential must not have Actions/Workflows or account administration permissions.

Exact Access OTP setup, bindings and release tests are in DEPLOYMENT.md. Cindy's supplied login is baker@whiskbakedbycindy.com. Nate's email and the real Access application's AUD must be confirmed; the live account has not been verified by these code tests.
...
`assets/js/cart.js` is a non-authoritative browser cart. Its public data is limited to visible item IDs, names, prices, sold-out status and public image information. It holds no customer/payment/admin credentials. It cannot place orders, reserve stock or publish edits. There is no new public write endpoint. The existing Access verification remains intact; hosted publishing is gated separately by explicit runtime configuration.

Hosted phone publishing now has a server implementation, pending live configuration and acceptance testing. Square order creation remains unimplemented. See BACKEND-NEXT-STEPS.md for the required server boundaries, configuration and acceptance tests. Future customer checkout belongs on a dedicated public commerce route, separate from privileged `/api/` editor routes; admin Access must never be bypassed to make customer checkout work.
